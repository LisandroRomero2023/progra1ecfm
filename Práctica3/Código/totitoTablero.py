...
'''
    ...
    def Ganador(self, jugador1, jugador2):
        pos= self.getPosicionesGanadorasHor() + self.getPosicionesGanadorasVer() + self.getPosicionesGanadorasDia()
        cont= 0
        m1 = jugador1.getCaracter()
        m2 = jugador2.getCaracter()
        for i in pos:
            if i == [m1, m1, m1]:
                self.setGanador(jugador1)
                cont +=1
                return True
            elif i == [m2, m2, m2]:
                self.setGanador(jugador2)
                cont +=1
                return True
        if cont == 0:
            return False
    
    def advertencias(self, jugador1, jugador2):
        cont=0
    ...
    def victoria(self, jugador1, jugador2):
        if self.Ganador(jugador1, jugador2) == True or self.advertencias(jugador1, jugador2) == True:
            return True
        else:
            return False
    ...
    def setPosicionCPU(self, fila, columna, jugador): ## Funciona bine, cambia el valor de la posición que la máquina selecciona
        while self.posiciones[fila][columna] != ' ':
            m = self.posicionesLibres()
            fila, columna = int(m[0]), int(m[1])
        self.posiciones[fila][columna] = jugador.getCaracter()
    ...
    def jugadaRandom(self, jugador):
        ...
        self.setPosicionCPU(posRandom[0], posRandom[1], jugador)
